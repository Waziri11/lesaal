...
export const CAMPAIGN_MEDIA_FIELD_TYPES = ["image", "video"];
export const CAMPAIGN_FIELD_TYPES = ["text", "email", "tel", "textarea", "select", ...CAMPAIGN_MEDIA_FIELD_TYPES];
export const PUBLIC_CAMPAIGNS_CACHE_TAG = "public-campaigns";
...
function normalizeAutoResponseTemplateValue(value) {
  return String(value ?? "").trim();
}

export function isCampaignMediaFieldType(type) {
  const normalized = String(type || "").trim().toLowerCase();
  return CAMPAIGN_MEDIA_FIELD_TYPES.includes(normalized);
}
...
function normalizeCampaignResponseMediaValue(value, expectedType = null) {
  if (value == null || value === "") {
    return null;
  }
  ...
  if (!isCampaignMediaFieldType(mediaType)) {
    return null;
  }
  ...
  if (!url || !mimeType || !filename || !originalName || !Number.isFinite(size) || size <= 0) {
    return null;
  }
...
export function sanitizeCampaignPayload(payload) {
  ...
  const autoResponseEnabled = Boolean(payload?.autoResponseEnabled);
  const autoResponseSubject = normalizeAutoResponseTemplateValue(payload?.autoResponseSubject);
  const autoResponseBody = normalizeAutoResponseTemplateValue(payload?.autoResponseBody);

  if (autoResponseEnabled && !autoResponseSubject) {
    throw new Error("Auto response subject is required when auto response is enabled.");
  }

  if (autoResponseEnabled && !autoResponseBody) {
    throw new Error("Auto response body is required when auto response is enabled.");
  }
  ...
  return {
    ...
    autoResponseEnabled,
    autoResponseSubject: autoResponseSubject || null,
    autoResponseBody: autoResponseBody || null,
...
function serializeAdminCampaign(campaign) {
  ...
  return {
    ...
    autoResponseEnabled: Boolean(campaign.autoResponseEnabled),
    autoResponseSubject: campaign.autoResponseSubject || "",
    autoResponseBody: campaign.autoResponseBody || "",
    autoResponseUpdatedAt: campaign.autoResponseUpdatedAt || null,
...
const getPublishedCampaignsCached = unstable_cache(
  async () => loadPublishedCampaigns(),
  ["public-campaigns:list"],
  {
    revalidate: 60,
    tags: [PUBLIC_CAMPAIGNS_CACHE_TAG],
  }
);

export async function getPublishedCampaigns({ bypassCache = false } = {}) {
  if (bypassCache) {
    return loadPublishedCampaigns();
  }

  return getPublishedCampaignsCached();
}
...
async function loadCampaignBySlugForPublic(slug) {
  await ensureDatabaseReady();
...
const getCampaignBySlugForPublicCached = unstable_cache(
  async (slug) => loadCampaignBySlugForPublic(slug),
...
export async function getCampaignBySlugForPublic(slug, { bypassCache = false } = {}) {
  if (bypassCache) {
    return loadCampaignBySlugForPublic(slug);
  }

  return getCampaignBySlugForPublicCached(slug);
}
...
export async function createCampaign(payload) {
  ...
  const created = await prisma.campaign.create({
    data: {
      ...
      autoResponseEnabled: normalized.autoResponseEnabled,
      autoResponseSubject: normalized.autoResponseSubject,
      autoResponseBody: normalized.autoResponseBody,
      autoResponseUpdatedAt: normalized.autoResponseEnabled ? new Date() : null,
      order: nextOrder,
      questions: {
...
export function getCampaignResponseRecipientEmail(questions, responseData) {
  const data = responseData && typeof responseData === "object" ? responseData : {};
  const orderedQuestions = Array.isArray(questions) ? questions : [];

  const emailQuestionKeys = orderedQuestions
    .filter((question) => String(question?.type || "").toLowerCase() === "email")
  ...
  for (const [key, rawValue] of dataEntries) {
...
export function stringifyCampaignResponseValue(value) {
  if (value == null) {
    return "";
  }
  ...
  if (Array.isArray(value)) {
    return value
      .map((entry) => stringifyCampaignResponseValue(entry))
  ...
  if (typeof value === "object") {
    const mediaValue = normalizeCampaignResponseMediaValue(value);
    if (mediaValue) {
...
export function buildCampaignResponseTemplateVariables({ campaign, response, questions }) {
  ...
  const variables = {
    ...
    target_market: String(campaign?.targetMarket || ""),
    submitted_at: response?.submittedAt ? new Date(response.submittedAt).toISOString() : "",
  };

  for (const [key, rawValue] of Object.entries(responseData)) {
    variables[String(key)] = stringifyCampaignResponseValue(rawValue);
  }
...
export function normalizeCampaignResponseData(questions, submittedData) {
  ...
  for (const question of questions) {
    ...
    if (isCampaignMediaFieldType(questionType)) {
      const mediaValue = normalizeCampaignResponseMediaValue(rawValue, questionType);

      if (question.required && !mediaValue) {
        missingLabels.push(question.label);
      }

      if (rawValue != null && rawValue !== "" && !mediaValue) {
        throw new Error(`Invalid media upload provided for ${question.label}.`);
      }

      cleaned[question.key] = mediaValue;
      continue;
    }

    if (rawValue != null && typeof rawValue === "object") {
      throw new Error(`Invalid response value provided for ${question.label}.`);
    }
...
export function toCsvHeaderString(columns) {
  return columns.map((column) => escapeCsvCell(column.label)).join(",");
}

export function toCsvRowString(columns, row) {
  return columns.map((column) => escapeCsvCell(row?.[column.key])).join(",");
}

export function toCsvString(columns, rows) {
  const headerRow = toCsvHeaderString(columns);
  const bodyRows = rows.map((row) => toCsvRowString(columns, row));
  return [headerRow, ...bodyRows].join("\n");
}

const DEFAULT_ADMIN_RESPONSES_LIMIT = 50;
const MAX_ADMIN_RESPONSES_LIMIT = 100;

function normalizeCampaignResponsePagination(options = {}) {
  const parsedLimit = Number.parseInt(String(options.limit ?? DEFAULT_ADMIN_RESPONSES_LIMIT), 10);
  const limit = Number.isFinite(parsedLimit)
    ? Math.min(Math.max(parsedLimit, 1), MAX_ADMIN_RESPONSES_LIMIT)
    : DEFAULT_ADMIN_RESPONSES_LIMIT;
  const cursor = options.cursor ? String(options.cursor).trim() : "";
  return { limit, cursor: cursor || null };
}

function serializeCampaignResponse(response) {
  return {
    id: response.id,
...
export async function getCampaignResponsesForAdmin(campaignId, options = {}) {
  await ensureDatabaseReady();
  const { limit, cursor } = normalizeCampaignResponsePagination(options);
  const includeAllResponses = Boolean(options.includeAllResponses);
  ...
  if (includeAllResponses) {
    responses = await prisma.campaignResponse.findMany(responseQuery);
  } else {
    const pageRows = await prisma.campaignResponse.findMany({
      ...responseQuery,
      take: limit + 1,
    ...
    responses = hasMore ? pageRows.slice(0, limit) : pageRows;
    nextCursor = hasMore ? responses[responses.length - 1]?.id || null : null;
  }
  ...
  return {
    ...
    responses: responses.map(serializeCampaignResponse),
    nextCursor,
    hasMore,
  };
}
