...
function isUniqueRateLimitKeyError(error) {
  ...
  const message = String(error?.message || "").toLowerCase();
  return message.includes("ratelimitentry_key_key") || message.includes("key");
}

let hasWarnedMissingRateLimitTable = false;

export async function consumeRateLimit({ key, limit, windowMs, lockMs = 0, denyOnMissingTable = false }) {
  await ensureDatabaseReady();

  const now = new Date();
  const windowStart = getWindowStart(now, windowMs);
  const fallbackRetryAfterSeconds = secondsUntil(getWindowResetAt(windowStart, windowMs), now);
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      if (!entry) {
        try {
          ...
          // Another request may have created this row concurrently.
          if (!isUniqueRateLimitKeyError(error)) {
            throw error;
          }

          entry = await tx.rateLimitEntry.findUnique({ where: { key } });
    ...
    return {
      allowed: !denyOnMissingTable,
      retryAfterSeconds: denyOnMissingTable ? fallbackRetryAfterSeconds : 0,
      remaining: denyOnMissingTable ? 0 : limit,
    };
  }
}
