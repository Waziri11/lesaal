...
const REPLACEMENT_HERO_IMAGE = "/images/services/social-media-management.jpg";
const REPLACEMENT_COMMENTARY_IMAGE = "/images/services/content-creation.jpg";
  ...
  switch (type) {
    case "HERO":
      return {
        ...
        primaryCtaLink: String(raw.primaryCtaLink || raw.ctaLink || "#campaign-form"),
        secondaryCtaText: String(raw.secondaryCtaText || "Demo"),
        secondaryCtaLink: String(raw.secondaryCtaLink || "#services-grid"),
    ...
    case "SERVICES_GRID":
      return {
        ...
        seeAllLink: String(raw.seeAllLink || "/services"),
    ...
    case "CAMPAIGN_FORM":
      return {
        heading: String(raw.heading || "Campaign form"),
        subheading: String(raw.subheading || raw.body || ""),
        submitText: String(raw.submitText || raw.ctaText || "Submit"),
        successMessage: String(raw.successMessage || "Submission received."),
        textStyle: sanitizeTextStyle(raw.textStyle),
      };
    case "FOOTER":
      return {
        ...
        ctaLink: String(raw.ctaLink || raw.primaryCtaLink || "#campaign-form"),
        contactEmail: String(raw.contactEmail || "leilawaziri@lesaal.com"),
        textStyle: sanitizeTextStyle(raw.textStyle),
      };
    default:
      return raw;
...
export async function ensureLandingConfig() {
  ...
  if (existingConfig && hasLegacySectionTypes(existingConfig)) {
...
export async function updateLandingConfig(payload) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const existingSections = await tx.landingSection.findMany({
    ...
    const incomingExistingIds = new Set(
      normalized.sections.map((section) => section.id).filter((id) => typeof id === "string" && existingSectionIds.has(id))
    );
    ...
    if (sectionIdsToDelete.length) {
      await tx.landingSection.deleteMany({ where: { id: { in: sectionIdsToDelete } } });
    }
    ...
    for (let index = 0; index < normalized.sections.length; index += 1) {
      ...
      if (sectionId && existingSectionIds.has(sectionId)) {
        ...
        const created = await tx.landingSection.create({
          data: {
            configId: config.id,
            type: section.type,
            title: section.title,
      ...
      const persistedSection = existingSections.find((existing) => existing.id === sectionId);
      const existingItems = persistedSection?.items || [];
      const existingItemIds = new Set(existingItems.map((item) => item.id));
      const incomingItemIds = new Set(
        section.items.map((item) => item.id).filter((id) => typeof id === "string" && existingItemIds.has(id))
      );
      ...
      if (itemIdsToDelete.length) {
        await tx.sectionItem.deleteMany({ where: { id: { in: itemIdsToDelete } } });
      }
    ...
    const existingFields = await tx.campaignFormField.findMany({ where: { configId: config.id } });
    const existingFieldIds = new Set(existingFields.map((field) => field.id));
    const incomingFieldIds = new Set(
      normalized.formFields
        .map((field) => field.id)
        .filter((id) => typeof id === "string" && existingFieldIds.has(id))
    );
    ...
    if (fieldIdsToDelete.length) {
      await tx.campaignFormField.deleteMany({ where: { id: { in: fieldIdsToDelete } } });
    }

    for (let index = 0; index < normalized.formFields.length; index += 1) {
      const field = normalized.formFields[index];
      const fieldData = {
        key: field.key,
        label: field.label,
        type: field.type,
      ...
      if (field.id && existingFieldIds.has(field.id)) {
        await tx.campaignFormField.update({
          where: { id: field.id },
          data: fieldData,
        });
      } else {
        await tx.campaignFormField.create({
          data: {
            configId: config.id,
            ...fieldData,
          },
